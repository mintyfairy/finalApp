...
<script type="text/javascript">
    function check() {
    	...
    	f.action="${pageContext.request.contextPath}/admin/cs/notice/${mode}";
			...
			<table class="table">
				<tr> 
					<td align="center">
						...
						<button type="button" class="btn" onclick="location.href='${pageContext.request.contextPath}/admin/cs/notice/list';">${mode=='update'?'수정취소':'등록취소'}</button>
						<c:if test="${mode=='update'}">
...
<c:if test="${mode=='update'}">
	<script type="text/javascript">
		$('.delete-file').click(function(){
			...
			let url = '${pageContext.request.contextPath}/admin/cs/notice/deleteFile';
			
			$.ajaxSetup({ beforeSend: function(e) { e.setRequestHeader('AJAX', true); } });
			$.post(url, {fileNum:fileNum}, function(data){
				$($tr).remove();
			}, 'json').fail(function() { //json방식 서버에서 제이슨 쏴야 함 없으면 텍스트 방식~
				alert('error...');				
			});
		});
	</script>
