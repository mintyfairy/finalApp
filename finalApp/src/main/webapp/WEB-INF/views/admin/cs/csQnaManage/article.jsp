...
function deleteInquiry(num) {
	if(confirm("문의를 삭제 하시겠습니까 ?")) {
		...
		let url = "${pageContext.request.contextPath}/admin/cs/qna/delete?"+query;
		location.href = url;
	}
}
...
function deleteAnswer(num) {
	if(confirm("답변을 삭제 하시겠습니까 ?")) {
		let query = "num="+num+"&${query}";
		let url = "${pageContext.request.contextPath}/admin/cs/qna/deleteAnswer?"+query;
		location.href = url;
	}
}
...
function sendAnswerOk() {
	...
	f.action = "${pageContext.request.contextPath}/admin/cs/qna/answer";
	f.submit();
}
</script>
    ...
    <div class="body-main">
		...
		<table class="table">
			<tr>
				...
				<td align="right">
					<button type="button" class="btn" onclick="location.href='${pageContext.request.contextPath}/admin/cs/qna/list?${query}';">리스트</button>
				</td>
			</tr>
		</table>
